...
const DocumentChunksStatus: React.FC<DocumentChunksStatusProps> = ({
  ...
  const previewLength = (document.contentPreview || '').length;
  const isLikelyPreviewOnly = stats && document.fileSize > 100000 && stats.total <= 1 && previewLength < 1000;
    ...
    <div className="mt-3 pt-3 border-t border-gray-200">
      ...
      {isLikelyPreviewOnly && (
        <div className="mb-2 p-2 bg-orange-50 border border-orange-200 rounded text-xs text-orange-700">
          ⚠️ 此文档可能只有预览内容（{previewLength} 字符），建议删除后重新上传完整文件以获得更好的检索效果。如果是 PDF/Word 文档，请确保文档包含可提取的文本（非扫描图片）。
        </div>
      )}
      {hasNoChunks ? (
        <button
