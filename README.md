# 英伟达网络产品AI 知识助手 (AI Knowledge Assistant for NVIDIA NETWORK PRODUCT)

一个基于 RAG (检索增强生成) 技术的智能知识库问答系统。专为保护用户隐私设计，采用**混合存储架构**：企业/团队知识库在云端共享，而用户的个人对话记录严格保存在本地浏览器中。

![License](https://img.shields.io/badge/license-MIT-blue.svg)
![React](https://img.shields.io/badge/react-18.x-61dafb.svg)
![TypeScript](https://img.shields.io/badge/typescript-5.x-3178c6.svg)
![Node](https://img.shields.io/badge/node-18%2B-339933.svg)

## ✨ 核心特性

- **🔒 隐私优先设计**：
  - **知识库共享**：上传的文档存储在私有服务器，团队成员可共享访问。
  - **对话私有**：聊天记录仅保存在用户浏览器的 LocalStorage，绝不上传云端，且支持隐身模式下的“阅后即焚”。

- **📚 智能知识库**：
  - **多格式支持**：支持 PDF, Word (.docx), Markdown (.md), TXT 等格式。
  - **智能解析**：内置 OCR 和 PDF 解析引擎，自动提取文本内容。
  - **语义切片**：采用父子块 (Parent-Child Chunking) 策略，保留上下文语义。

- **🧠 强大的检索能力**：
  - **混合检索 (Hybrid Search)**：结合向量语义检索 (Embedding) 和关键词匹配 (Keyword Matching)，确保高召回率。
  - **意图识别 (Intent Detection)**：
    - **命令类**：优先匹配配置命令和 CLI 语法。
    - **概念类**：优先匹配定义、解释性段落（如 "What is MLAG"）。
    - **故障排查类**：优先匹配 Troubleshooting 指南和解决方案。
  - **结果融合 (RRF)**：使用 Reciprocal Rank Fusion 算法融合多路检索结果，智能平衡精确匹配与语义相关性。

- **🧪 内置基准测试**：
  - 提供 `npm run test:benchmark` 工具，用于评估检索系统的准确率（Precision）和召回能力。
  - 支持自定义测试用例，确保系统在特定领域的表现。

- **🤖 多模型支持**：
  - 接入 SiliconFlow API，支持 Qwen (通义千问)、DeepSeek 等高性能大模型。
  - 支持流式响应 (Streaming)，提供流畅的对话体验。

## 🛠️ 技术栈

### 前端 (Frontend)
- **框架**: React 18 + TypeScript
- **构建工具**: Vite
- **状态管理**: Zustand
- **样式**: Tailwind CSS + Lucide React
- **Markdown渲染**: React Markdown + Syntax Highlighting

### 后端 (Backend)
- **运行时**: Node.js
- **Web框架**: Express
- **文件处理**: Multer, PDF-Parse, Mammoth
- **向量模型**: Qwen-Embedding (via SiliconFlow)

### 数据存储 (Storage)
- **Server**: 文件系统 (JSON) 存储知识库文档和向量索引。
- **Client**: LocalStorage 存储用户会话、消息记录和个人设置。

## 🚀 快速开始

### 前置要求
- Node.js >= 18.0.0
- npm 或 pnpm

### 1. 克隆项目
```bash
git clone https://github.com/your-repo/ai-knowledge-assistant.git
cd ai-knowledge-assistant
```

### 2. 安装依赖
```bash
npm install
```

### 3. 配置环境变量
复制 `.env.example` 为 `.env` (如果不存在，请手动创建)，并配置以下必要参数：

```env
# 硅基流动 API Key (用于 LLM 和 Embedding)
VITE_SILICONFLOW_API_KEY=your_api_key_here

# 后端服务器地址 (默认)
VITE_API_SERVER_URL=http://localhost:8787
```

### 4. 启动服务
项目包含前端和后端两个服务，需要同时启动。

**方式一：一键启动（推荐）**
```bash
# 同时启动前端和后端
npm run dev
# 注意：你需要手动在另一个终端运行 npm run server，或者配置 concurrently
```

**方式二：分别启动**

终端 1 (后端服务 - 端口 8787):
```bash
npm run server
```

终端 2 (前端页面 - 端口 5173/5174):
```bash
npm run dev
```

打开浏览器访问 `http://localhost:5173` 即可使用。

## 📂 项目结构

```
src/
├── components/       # UI 组件 (聊天界面、知识库管理等)
├── lib/             # 核心逻辑库
│   ├── aiModels.ts          # LLM 模型接口
│   ├── retrieval.ts         # RAG 检索逻辑
│   ├── unifiedStorage.ts    # 统一存储接口 (前端)
│   ├── localStorage.ts      # 本地隐私存储 (对话记录)
│   └── serverStorage.ts     # 服务器存储适配器
├── stores/          # Zustand 状态管理
└── server/          # 后端服务代码 (Express)
    ├── index.mjs    # API 入口
    └── storage.mjs  # 服务端文件存储逻辑
```

## ⚠️ 注意事项

1.  **数据持久性**：
    *   **对话记录**：存储在浏览器本地。**清除浏览器缓存**或使用**隐身模式**会导致对话记录丢失。
    *   **知识库**：存储在 `server/data` 目录下。请定期备份该目录以防服务器数据丢失。

2.  **API 配额**：
    *   本项目依赖 SiliconFlow API 进行 Embedding 和文本生成，请确保您的 API Key 有足够的配额。

## 🤝 贡献指南

欢迎提交 Issue 和 Pull Request！

1.  Fork 本仓库
2.  创建特性分支 (`git checkout -b feature/AmazingFeature`)
3.  提交更改 (`git commit -m 'Add some AmazingFeature'`)
4.  推送到分支 (`git push origin feature/AmazingFeature`)
5.  开启 Pull Request

## 📄 许可证

MIT License
